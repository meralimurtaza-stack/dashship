...
const Card: FC<CardProps> = ({ rec, status, onApprove, onSkip }) => {
  ...
  return (
    <div
      className={`border border-ds-border bg-ds-surface p-4 transition-opacity duration-200 ${
    ...
    >
      ...
      {/* Actions */}
      {isApproved ? (
        <p className="font-mono text-[10px] text-ds-success uppercase tracking-wide">
          Applied ✓
        </p>
      ) : (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-2">
        <div className="w-5 h-5 shrink-0 bg-ds-accent flex items-center justify-center mt-0.5">
      ...
      {hasPending && recommendations.length > 1 && (
        <button
          ...
          className="w-full border border-ds-accent text-ds-accent font-mono text-[10px] uppercase tracking-wide px-4 py-2 hover:bg-ds-accent hover:text-white transition-colors"
        >
          Approve All ✓
        </button>
      )}
      ...
      {allHandled && (
        <div className="border border-ds-border bg-ds-surface p-5 text-center space-y-3">
          <div className="flex items-center justify-center gap-2">
            ...
            <p className="font-mono text-xs font-medium text-ds-text">
              Data looks clean
            </p>
          </div>
          <button
            onClick={onStartPlanning}
            className="bg-ds-accent text-white font-mono text-xs font-medium px-6 py-2.5 hover:bg-ds-accent-hover transition-colors inline-flex items-center gap-2"
          >
            Start Planning
